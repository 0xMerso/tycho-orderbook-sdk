...
pub trait OrderbookSolver: Send + Sync {
    ...
    fn optimize(&self, protosims: &[ProtoSimComp], steps: Vec<f64>, eth_usd: f64, gas_price: u128, from: &SrzToken, to: &SrzToken, price: f64, worth: f64) -> Vec<TradeResult>;
}
...
impl OrderbookSolver for DefaultOrderbookSolver {
    ...
    fn optimize(&self, protosim: &[ProtoSimComp], steps: Vec<f64>, eth_usd: f64, gas_price: u128, from: &SrzToken, to: &SrzToken, price: f64, worth: f64) -> Vec<TradeResult> {
        tracing::debug!("Default solver: optimize called with steps: {:?}", steps);
        optimize(protosim, steps, eth_usd, gas_price, from, to, price, worth)
    }
}
...
pub fn optimize(protosim: &[ProtoSimComp], steps: Vec<f64>, eth_usd: f64, gas_price: u128, from: &SrzToken, to: &SrzToken, spot_price: f64, output_u_ethworth: f64) -> Vec<TradeResult> {
    let trades: Vec<TradeResult> = steps
        .par_iter()
        .enumerate()
        .map(|(x, amount)| {
            let tmstp = Instant::now();
            let result = maths::opti::gradient(*amount, protosim, from.clone(), to.clone(), eth_usd, gas_price, spot_price, output_u_ethworth);
