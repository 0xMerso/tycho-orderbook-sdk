...
pub async fn simulate<S: OrderbookSolver>(
    ...
    match body.point {
        ...
        None => {
            let steps = solver.generate_steps(adjusted_aggb_base);
            let bids = solver.optimize(&pcsdata, steps.clone(), eth_usd, gas_price, &base, &quote, price_base_to_quote, quote_worth_eth);
            result.bids = bids;
            tracing::trace!(" 🔄  Bids done, now switching to asks");
            let steps = solver.generate_steps(adjusted_aggb_quote);
            let asks = solver.optimize(&pcsdata, steps.clone(), eth_usd, gas_price, &quote, &base, price_quote_to_base, base_worth_eth);
            result.asks = asks;
        }
    }
    result
...
pub fn compute_best_trade(pcs: &[ProtoSimComp], eth_usd: f64, gas_price: u128, from: &SrzToken, to: &SrzToken, amount: f64, spot_price: f64, output_u_ethworth: f64) -> TradeResult {
